...
module.exports = Joi.object({
  ...
  gameDetection: Joi.object({
    enabled: Joi.boolean().label('Game Detection Enabled').default(true),
    checkInterval: Joi.number().min(1).label('Check Interval').default(5),
    defaultDetails: Joi.string().min(2).max(128).label('Default Details'),
    defaultState: Joi.string().min(2).max(128).label('Default State'),
    formatString: Joi.string().min(2).max(128).label('Format String').default('Playing: {game}'),
    autoFetchImage: Joi.boolean().label('Auto Fetch Image').default(true),
  }),
})
